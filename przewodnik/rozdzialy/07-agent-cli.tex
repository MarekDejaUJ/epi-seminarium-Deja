\chapter{Praca z agentem programistycznym}\label{ch:agent}

\termin{Agent programistyczny}, nazywany też agentem SI, to program oparty na dużym modelu językowym, uruchamiany z terminala w katalogu projektu. Dostaje od Ciebie cel opisany słowami, po czym sam czyta pliki projektu, pisze i zmienia kod, uruchamia testy, czyta ich wyniki i poprawia to, co nie przeszło. Pracuje w pętli i sam decyduje, które pliki otworzyć.

W rozdziale opisana jest metoda budowy pakietu z użyciem takiego agenta. Granice dozwolonego użycia i sposób ich dokumentowania są w rozdziale~\ref{ch:etyka}; tutaj chodzi o to, \wyroznienie{jak} pracować, żeby powstało narzędzie badawcze, za które da się odpowiadać.

Nazywamy rzecz po imieniu, bo seminarium wymaga jawności użycia. Praca, która opisuje agenta jako \enquote{narzędzie wspomagające}, zaciera to, co recenzent ma ocenić: gdzie kończy się wkład autora, a gdzie zaczyna wynik działania programu.

Jedno zastrzeżenie na wstępie: \wyroznienie{pakiet budujesz zawsze lokalnie}, niezależnie od tego, gdzie piszesz pracę. Overleaf składa dokumenty i nie uruchamia R-a, więc kod, testy i sprawdzenie pakietu dzieją się u Ciebie na komputerze. Podział na dwie ścieżki z podrozdziału~ef{sec:sciezki} dotyczy tekstu pracy, nie projektu.

Punkt wyjścia jest taki: implementacja algorytmu z artykułu metodycznego to zadanie, w którym trudność nie leży w składni języka, tylko w zrozumieniu metody. Agent zdejmuje z Ciebie pierwsze i \wyroznienie{nie zdejmuje drugiego}. Twoja rola przesuwa się z pisania linii kodu na projektowanie kontraktu i sprawdzanie, czy wynik ten kontrakt spełnia. To trudniejsza rola, nie łatwiejsza.
...
Warunkiem sensownej pracy jest to, żeby agent \wyroznienie{sam uruchamiał testy}. Bez tego dostajesz kod, który wygląda poprawnie, i musisz sprawdzać go ręcznie. Z tym dostajesz kod, który przechodzi testy napisane przez Ciebie wcześniej.
...
\subsection{Podpowiadanie w edytorze to co innego}
...
Rozróżnienie ma skutek praktyczny dla rejestru poleceń. Podpowiedzi w edytorze nie rejestrujesz, bo nie ma czego: nie ma polecenia ani wyodrębnionej zmiany. Każde polecenie wydane agentowi rejestrujesz. Granica przebiega tam, gdzie \wyroznienie{istnieje polecenie, które da się przytoczyć}.
...
\wyroznienie{Krok pierwszy: specyfikacja.} Czytasz artykuł źródłowy i spisujesz kontrakt: co wchodzi, w jakich dziedzinach, jakie warunki muszą zachodzić, co wychodzi, jakie własności muszą być spełnione. Szablon dokumentu znajdziesz w materiałach seminarium. Ten dokument piszesz sam, bez agenta, bo powstaje z lektury, a nie z kodu.

\wyroznienie{Krok drugi: testy.} Z przypadków wypisanych w specyfikacji robisz testy. Każdy przypadek analityczny ma wynik policzony ręcznie ze wzoru. Testy piszesz \wyroznienie{przed} implementacją i uruchamiasz je: mają zawieść, bo funkcji jeszcze nie ma. To brzmi absurdalnie, dopóki nie zobaczysz, jak łatwo napisać test, który przechodzi zawsze.

\wyroznienie{Krok trzeci: implementacja.} Dopiero teraz uruchamiasz agenta, dając mu specyfikację i testy jako kontekst. Polecenie brzmi: zaimplementuj funkcję tak, żeby przechodziła te testy, nie zmieniając testów.

\wyroznienie{Krok czwarty: dokumentacja.} Bloki dokumentacyjne powstają ze specyfikacji, a nie z kodu. Opis argumentu ma mówić, co argument znaczy, a nie jakiego jest typu.

\subsection{Dlaczego odwrotna kolejność zawodzi}

Kiedy najpierw powstaje kod, a testy potem, testy powstają \wyroznienie{z obserwacji zachowania kodu}. Jeżeli implementacja zawiera błąd, test ten błąd utrwala. Sprawdzasz wtedy, czy kod się nie zmienił, a nie czy liczy poprawnie.

To rozróżnienie jest sednem całej metody i wraca w rozdziale~\ref{ch:jakpisac} jako wymóg wobec rozdziału drugiego pracy. Test, którego oczekiwana wartość pochodzi z uruchomienia własnego kodu, nie jest dowodem poprawności.

\section{Pętla naprawcza}
...
Twoja rola w tym cyklu nie polega na czekaniu. Polega na obserwowaniu, \wyroznienie{co agent zmienia}, żeby doprowadzić do przejścia testów.

\subsection{Kiedy przerwać}

Są trzy sytuacje, w których pętlę trzeba zatrzymać, bo dalsze krążenie tylko pogarsza kod.

\wyroznienie{Agent zmienia test zamiast kodu.} Test jest zapisem tego, co metoda ma robić. Zmiana testu, żeby przechodził, jest zmianą definicji problemu. Jeżeli test jest błędny, poprawiasz go sam, po sprawdzeniu w artykule, i odnotowujesz to w specyfikacji.

\wyroznienie{Agent osłabia warunek wstępny.} Sprawdzenie, które zatrzymywało wykonanie przy danych spoza dziedziny, znika albo zamienia się w ostrzeżenie. Funkcja zaczyna zwracać wynik tam, gdzie powinna odmówić działania. To najgroźniejszy rodzaj regresu, bo testy przechodzą, a pakiet staje się niebezpieczny.

\wyroznienie{Agent dodaje zależność.} Zamiast rozwiązać problem, sięga po gotową funkcję z pakietu, którego w projekcie nie ma. Każda zależność zwiększa ryzyko, że pakiet przestanie się instalować, i wymaga Twojej świadomej decyzji.

\subsection{Miejsca, w których agent zgaduje}
...
\begin{tabelaepi}{Typowe miejsca rozejścia się implementacji z metodą}{tab:zgadywanie}
...
\subsection{Wzór na kod}

Najczęstsze polecenie w tym seminarium. Kluczowe jest podanie wzoru \wyroznienie{razem z objaśnieniem symboli} oraz wskazanie, że wynik ma być liczony na wektorach, a nie w pętli.
...
\subsection{Dane, na których metoda nie ma sensu}

Polecenie do budowy testów odporności. Sedno polega na tym, żeby prosić o \wyroznienie{dane}, a nie o testy: testy piszesz sam, bo to Ty decydujesz, jaki wynik jest poprawny.
...
\subsection{Diagnostyka zamiast zgadywania}
...
\subsection{Dokumentacja ze specyfikacji}
...
\section{Rejestr poleceń}\label{sec:rejestr}

Wykaz poleceń jest wymaganym aneksem pracy, ale prowadzi się go \wyroznienie{od pierwszego dnia}, a nie odtwarza z pamięci przed oddaniem. Odtworzenie jest niewykonalne: po pół roku nie pamięta się, które polecenie doprowadziło do której funkcji.
...
\begin{tabelaepi}{Wpis w rejestrze poleceń}{tab:wpisrejestr}
...
Rejestr ma jeszcze jedno zastosowanie, ważniejsze od formalnego. Wpis, w którym kolumna \enquote{zmiany własne} jest pusta, oznacza kod, którego nie sprawdziłeś. Przed obroną warto przejrzeć rejestr pod tym kątem.

\section{Co robisz sam}
...
\begin{itemize}
  \item lektura artykułu źródłowego i spisanie specyfikacji;
  \item rozstrzygnięcie niedopowiedzeń artykułu wraz z uzasadnieniem;
  \item wyliczenie ręczne przypadków analitycznych;
  \item decyzja o obsłudze braków danych i wartości brzegowych;
  \item projekt procedury generowania danych;
  \item decyzja o każdej zależności pakietu;
  \item interpretacja wyników w studium przypadku;
  \item cały tekst pracy.
\end{itemize}

Sprawdzian na koniec: \wyroznienie{musisz umieć objaśnić każdą linię kodu, którą oddajesz}. Nie chodzi o pamiętanie składni, tylko o umiejętność powiedzenia, co ta linia liczy i dlaczego akurat tak. Linia, której nie umiesz objaśnić, jest linią, której w Twoim pakiecie nie powinno być.
